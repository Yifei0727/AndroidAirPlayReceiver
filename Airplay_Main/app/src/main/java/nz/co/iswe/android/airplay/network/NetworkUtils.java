...
public class NetworkUtils {
    ...
    public String getHardwareAddressString(InetAddress inetAddress) {
        try {
            /* Search network interfaces for an interface with a valid, non-blocked hardware address */
            for (final NetworkInterface iface : Collections.list(NetworkInterface.getNetworkInterfaces())) {
                if (iface.isLoopback()) {
                ...
                Enumeration<InetAddress> ifaceInetAddresses = iface.getInetAddresses();
                boolean isFound = false;
                while (ifaceInetAddresses.hasMoreElements()) {
                    InetAddress addr = ifaceInetAddresses.nextElement();
