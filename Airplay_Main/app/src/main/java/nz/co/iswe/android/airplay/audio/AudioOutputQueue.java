...
public class AudioOutputQueue implements AudioClock {
    private static final Logger LOG = Logger.getLogger(AudioOutputQueue.class.getName());

    private static final double QUEUE_LENGTH_MAX_SECONDS = 10;
     ...
     * <p>
     * It is used to detect the gaps between the packets and
     * as the number of silence frames
     * to write on a queue underrate
     */
...
//		audioTrack = new AudioTrack(streamType, sampleRateInHz, channelConfig, audioFormat, bufferSizeInBytes, mode);
        ...
        LOG.info("AudioTrack created successfully with a buffer of : " + bufferSizeInBytes + " bytes and : " + bufferSizeInBytes / bytesPerFrame + " frames.");
...
//		mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, (int) (mVolumeVaule / scale), 0);
    ...
    public synchronized boolean enqueue(final long frameTime, final byte[] frames) {
        ...
        if (delay < -packetSeconds) {
            ...
            return false;
        } else if (delay > QUEUE_LENGTH_MAX_SECONDS) {
