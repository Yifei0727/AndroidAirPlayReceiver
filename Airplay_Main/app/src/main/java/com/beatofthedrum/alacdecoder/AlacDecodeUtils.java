...
public class AlacDecodeUtils {
    ...
    static int read_bits_16(AlacFile alac, int bits) {
        int result = 0;
    ...
    static int read_bits(AlacFile alac, int bits) {
        int result = 0;

        if (bits > 16) {
            bits -= 16;

            result = read_bits_16(alac, 16) << bits;
        }

        result |= read_bits_16(alac, bits);

        return result;
    }

    /* reads a single bit */
    static int read_bit(AlacFile alac) {
        int result = 0;
        int new_accumulator = 0;
        int part1 = 0;
    ...
    static void unread_bits(AlacFile alac, int bits) {
    ...
    public static int entropy_decode_value(AlacFile alac, int readSampleSize, int k, int rice_kmodifier_mask) {
        ...
        while (x <= Defines.RICE_THRESHOLD && read_bit(alac) != 0) {
            x++;
        }
        ...
        if (x > Defines.RICE_THRESHOLD) {
            ...
            value = read_bits(alac, readSampleSize);
            ...
            if (k != 1) {
                int extraBits = read_bits(alac, k);
    ...
    public static int decode_frame(AlacFile alac, byte[] inbuffer, int[] outbuffer, int outputsize) {
        ...
        channels = read_bits(alac, 3);

        outputsize = outputsamples * alac.bytes_per_sample;
        ...
        if (channels == 0) // 1 channel
        {
            ...
            read_bits(alac, 12); // unknown, skip 12 bits

            hassize = read_bits(alac, 1); // the output sample size is stored soon

            uncompressed_bytes = read_bits(alac, 2); // number of bytes in the (compressed) stream that are not compressed

            isnotcompressed = read_bits(alac, 1); // whether the frame is compressed

            if (hassize != 0) {
                /* now read the number of samples,
                 * as a 32bit integer */
                outputsamples = read_bits(alac, 32);
                outputsize = outputsamples * alac.bytes_per_sample;
            }
            ...
            if (isnotcompressed == 0) { // so it is compressed
                ...
                prediction_type = read_bits(alac, 4);
                prediction_quantitization = read_bits(alac, 4);

                ricemodifier = read_bits(alac, 3);
                predictor_coef_num = read_bits(alac, 5);

                /* read the predictor table */

                for (i = 0; i < predictor_coef_num; i++) {
                    tempPred = read_bits(alac, 16);
                    if (tempPred > 32767) {
                ...
                if (uncompressed_bytes != 0) {
                    for (i = 0; i < outputsamples; i++) {
                        alac.uncompressed_bytes_buffer_a[i] = read_bits(alac, uncompressed_bytes * 8);
                    }
                }


                entropy_rice_decode(alac, alac.predict_error_buffer_a, outputsamples, readsamplesize, alac.setinfo_rice_initialhistory, alac.setinfo_rice_kmodifier, ricemodifier * (alac.setinfo_rice_historymult / 4), (1 << alac.setinfo_rice_kmodifier) - 1);

                if (prediction_type == 0) { // adaptive fir
                    alac.output_samples_buffer_a = predictor_decompress_fir_adapt(alac.predict_error_buffer_a, outputsamples, readsamplesize, predictor_coef_table, predictor_coef_num, prediction_quantitization);
                } else {
                ...
                if (alac.setinfo_sample_size <= 16) {
                    ...
                    for (int i = 0; i < outputsamples; i++) {
                        int audiobits = read_bits(alac, alac.setinfo_sample_size);
                        bitsmove = 32 - alac.setinfo_sample_size;

                        audiobits = ((audiobits << bitsmove) >> bitsmove);

                        alac.output_samples_buffer_a[i] = audiobits;
                    }
                } else {
                    int x;
                    int m = 1 << (24 - 1);
                    for (int i = 0; i < outputsamples; i++) {
                        ...
                        audiobits = read_bits(alac, 16);
                        /* special case of sign extension..
                         * as we'll be ORing the low 16bits into this */
                        audiobits = audiobits << (alac.setinfo_sample_size - 16);
                        audiobits = audiobits | read_bits(alac, alac.setinfo_sample_size - 16);
                        x = audiobits & ((1 << 24) - 1);
            ...
            switch (alac.setinfo_sample_size) {
                case 16: {
                    ...
                    for (int i = 0; i < outputsamples; i++) {
                        int sample = alac.output_samples_buffer_a[i];
                        outbuffer[i * alac.num_channels] = sample;
                        ...
                        outbuffer[(i * alac.num_channels) + 1] = 0;
                    }
                    break;
                }
                case 24: {
                    for (int i = 0; i < outputsamples; i++) {
                        ...
                        outbuffer[i * alac.num_channels * 3] = ((sample) & 0xFF);
                        outbuffer[i * alac.num_channels * 3 + 1] = ((sample >> 8) & 0xFF);
                        outbuffer[i * alac.num_channels * 3 + 2] = ((sample >> 16) & 0xFF);
                        ...
                        outbuffer[i * alac.num_channels * 3 + 3] = 0;
                        outbuffer[i * alac.num_channels * 3 + 4] = 0;
                        outbuffer[i * alac.num_channels * 3 + 5] = 0;
        ...
        } else if (channels == 1) // 2 channels
        {
            ...
            uncompressed_bytes = read_bits(alac, 2); // the number of bytes in the (compressed) stream that are not compressed
            ...
            if (isnotcompressed == 0) { // compressed
                ...
                interlacing_shift = read_bits(alac, 8);
                interlacing_leftweight = read_bits(alac, 8);

                /******** channel 1 ***********/
                prediction_type_a = read_bits(alac, 4);
                prediction_quantitization_a = read_bits(alac, 4);

                ricemodifier_a = read_bits(alac, 3);
                predictor_coef_num_a = read_bits(alac, 5);
                ...
                /******** channel 2 *********/
                prediction_type_b = read_bits(alac, 4);
                prediction_quantitization_b = read_bits(alac, 4);

                ricemodifier_b = read_bits(alac, 3);
                predictor_coef_num_b = read_bits(alac, 5);
                ...
                if (uncompressed_bytes != 0) { // see mono case
                    for (int i = 0; i < outputsamples; i++) {
                        alac.uncompressed_bytes_buffer_a[i] = read_bits(alac, uncompressed_bytes * 8);
                        alac.uncompressed_bytes_buffer_b[i] = read_bits(alac, uncompressed_bytes * 8);
                    }
                }

                /* channel 1 */

                entropy_rice_decode(alac, alac.predict_error_buffer_a, outputsamples, readsamplesize, alac.setinfo_rice_initialhistory, alac.setinfo_rice_kmodifier, ricemodifier_a * (alac.setinfo_rice_historymult / 4), (1 << alac.setinfo_rice_kmodifier) - 1);

                if (prediction_type_a == 0) { // adaptive fir

                    alac.output_samples_buffer_a = predictor_decompress_fir_adapt(alac.predict_error_buffer_a, outputsamples, readsamplesize, predictor_coef_table_a, predictor_coef_num_a, prediction_quantitization_a);
                ...
                /* channel 2 */
                entropy_rice_decode(alac, alac.predict_error_buffer_b, outputsamples, readsamplesize, alac.setinfo_rice_initialhistory, alac.setinfo_rice_kmodifier, ricemodifier_b * (alac.setinfo_rice_historymult / 4), (1 << alac.setinfo_rice_kmodifier) - 1);

                if (prediction_type_b == 0) { // adaptive fir
                    alac.output_samples_buffer_b = predictor_decompress_fir_adapt(alac.predict_error_buffer_b, outputsamples, readsamplesize, predictor_coef_table_b, predictor_coef_num_b, prediction_quantitization_b);
                } else {
                ...
                if (alac.setinfo_sample_size <= 16) {
                    ...
                    for (int i = 0; i < outputsamples; i++) {
                        ...
                        audiobits_a = read_bits(alac, alac.setinfo_sample_size);
                        audiobits_b = read_bits(alac, alac.setinfo_sample_size);
                        ...
                        alac.output_samples_buffer_a[i] = audiobits_a;
                        alac.output_samples_buffer_b[i] = audiobits_b;
                    }
                } else {
                    int x;
                    int m = 1 << (24 - 1);
                    ...
                    for (int i = 0; i < outputsamples; i++) {
                        ...
                        audiobits_a = read_bits(alac, 16);
                        audiobits_a = audiobits_a << (alac.setinfo_sample_size - 16);
                        audiobits_a = audiobits_a | read_bits(alac, alac.setinfo_sample_size - 16);
                        x = audiobits_a & ((1 << 24) - 1);
                        audiobits_a = (x ^ m) - m;        // sign extend 24 bits

                        audiobits_b = read_bits(alac, 16);
                        audiobits_b = audiobits_b << (alac.setinfo_sample_size - 16);
                        audiobits_b = audiobits_b | read_bits(alac, alac.setinfo_sample_size - 16);
            ...
            switch (alac.setinfo_sample_size) {
                case 16: {
                    deinterlace_16(alac.output_samples_buffer_a, alac.output_samples_buffer_b, outbuffer, alac.num_channels, outputsamples, interlacing_shift, interlacing_leftweight);
                    break;
                }
                case 24: {
                    deinterlace_24(alac.output_samples_buffer_a, alac.output_samples_buffer_b, uncompressed_bytes, alac.uncompressed_bytes_buffer_a, alac.uncompressed_bytes_buffer_b, outbuffer, alac.num_channels, outputsamples, interlacing_shift, interlacing_leftweight);
                    break;
                }
                case 20:
                case 32:
    ...
    public static AlacFile create_alac(int sampleSize, int numChannels) {
        AlacFile file = new AlacFile();

        file.sample_size = sampleSize;
        file.num_channels = numChannels;
        file.bytes_per_sample = ((sampleSize + 7) / 8) * numChannels;
